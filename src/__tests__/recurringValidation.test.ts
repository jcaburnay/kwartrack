...
describe("validateRecurring — inherited transaction rules", () => {
	it("rejects non-positive amount", () => {
		expect(validateRecurring(base({ amountCentavos: 0 })).ok).toBe(false);
		expect(validateRecurring(base({ amountCentavos: -1 })).ok).toBe(false);
	});

	it("rejects expense without tag", () => {
		const r = validateRecurring(base({ tagId: null }));
		expect(r.ok).toBe(false);
	});

	it("rejects income with from_account", () => {
		const r = validateRecurring(
			base({
				type: "income",
				fromAccountId: "from",
				toAccountId: "to",
	...
	it("accepts a valid income recurring", () => {
		expect(
			validateRecurring(
				base({
					type: "income",
	...
	it("accepts a valid transfer recurring with fee", () => {
		expect(
			validateRecurring(
				base({
					type: "transfer",
	...
	it("rejects transfer with non-positive fee", () => {
		const r = validateRecurring(
			base({
				type: "transfer",
				fromAccountId: "acc-1",
