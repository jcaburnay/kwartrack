...
| `credit` | Credit card. Has `creditLimit` and optional `installmentLimit`. Balance is outstanding debt — counted as a **liability**. See [Credit accounts](#credit-accounts) below. |
...
- **Type-specific fields:**
  ...
  - `credit.installmentLimit` — editable (ditto, and users may opt in/out of installment programs).
...
| --- | --- | --- |
| `creditLimit` | yes | Maximum standard credit available. Must be `> 0`. |
| `installmentLimit` | no | Optional separate installment-only credit pool. Set for cards with a dedicated installment tranche (e.g. BPI Bonus Madness Limit, BDO designated installment limit). Leave null for cards with one shared pool (most cards). Must be `≥ 0` when set; **no enforced relationship to `creditLimit`** (BPI Madness can be up to 100% bonus on top of the regular line). |
...
- `availableCredit` = `creditLimit − (balance − installment-linked portion)`
- `utilization` = used / `creditLimit`
- `installmentCommitted` = Σ(`remaining-occurrences × amount`) across all Recurring rows where `from = this card`
- `availableInstallment` = `installmentLimit − installmentCommitted` (only when `installmentLimit` is set)

**UI behavior:**

- *Accounts table.* Balance rendered with debt styling (red, or `(owed)` suffix). Compact utilization bar next to the balance: one bar when `installmentLimit` is null, two bars (regular + installment) when set — mirrors BPI's Madness-style two-bar layout.
- *Detail strip (when selected).* Shown between the accounts table and the transactions table: current balance, available credit, utilization bar(s). A **"Pay this card"** button opens a `New Transaction` modal pre-filled with `type = transfer`, `to = this card`; amount is left blank for the user to type.
...
2. **Form reveals the fields relevant to the chosen type:**
    ...
    - `credit`: `name`, `initial-balance`, `creditLimit`, `installmentLimit` (optional).
